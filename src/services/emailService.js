...
const kirimEmailSelamatDatang = async (emailTujuan, namaPengguna) => {
  try {
    ...
    if (sendGrid) {
      ...
      console.log('✅ Email selamat datang (SendGrid) berhasil dikirim ke:', emailTujuan);
    ...
    console.log('✅ Email selamat datang (SMTP) berhasil dikirim ke:', emailTujuan);
...
const testKoneksiEmail = async () => {
  try {
    if (sendGrid) {
      ...
      console.log('✅ SendGrid API tersedia (key terdeteksi)');
      return { sukses: true, provider: 'sendgrid' };
    }

    const transporter = buatTransporter();
    await transporter.verify();
    console.log('✅ Koneksi email server (SMTP) berhasil!');
